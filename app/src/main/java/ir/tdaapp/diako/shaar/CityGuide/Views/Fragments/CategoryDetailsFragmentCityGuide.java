    ...
    switch (v.getId()) {
      case R.id.categoryDetailsAddItemFab:
        if (userId == 0) {
          ...
          ((GuideActivity) getActivity()).onAddFragment(new AddItemFragmentCityGuide(),
            R.anim.fadein, R.anim.fadeout,
            true, AddItemFragmentCityGuide.TAG);
        }
        break;
      case R.id.imgCategoryDetailsFilter:
        presenter.getItemByFilter(searchBar.getText().toString(), selectedModel == null ? getArguments().getInt("ID") : selectedModel.getId(), 0);
        break;
