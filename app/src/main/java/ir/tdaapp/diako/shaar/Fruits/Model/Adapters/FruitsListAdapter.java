...
public class FruitsListAdapter extends RecyclerView.Adapter<FruitsListAdapter.ViewHolder> {
  ...
  public void onBindViewHolder(@NonNull FruitsListAdapter.ViewHolder holder, int position) {
    ...
    holder.name.setText(item.getName() + (item.isTypeEnum() ? " " + item.getRange() + " کیلو" : ""));
    holder.price.setText((int) item.getPrice() + " تومان");

    String imageUrl = FruitsBaseRepository.API_IMAGE + item.getImageUrl();
    Glide.with(context)
      .load(imageUrl)
